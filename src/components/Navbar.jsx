...
import { LayoutDashboard, ArrowUpRight, ChevronDown, XIcon, X } from 'lucide-react'
...
const NavItemWithMegaMenu = ({ title, links, imageText, imageSubtext, gradient, onTitleClick, onCardClick }) => (
  <div className="group relative py-4 font-['Playfair_Display',_Georgia,_serif]">
    <button 
      ...
      className="hover:text-black transition-colors uppercase cursor-pointer text-sm font-semibold tracking-wide text-gray-700"
    >
      {title}
    </button>
...
const Navbar = () => {
  ...
  const handleMailRedirect = (e) => {
    ...
    window.location.href = `mailto:webrevinfo@gmail.com?subject=${encodeURIComponent(subjectLine)}&body=${bodyText}`;
  ...
  const menuData = {
    ...
    contact: { id: 'contact', title: "Contact Us", gradient: "from-emerald-400 to-teal-600", imageText: "UI/UX Design Systems", imageSubtext: "Clean, user-focused interfaces.", links: [{title: "E-commerce Development", desc: "Online stores that convert"}, {title: "CMS Integration", desc: "Easy content management"}] },
    ...
    <div className="fixed top-6 left-0 right-0 z-50 flex justify-center w-full px-4 md:px-8 pointer-events-none font-['Playfair_Display',_Georgia,_serif]">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 pointer-events-auto font-['Playfair_Display',_Georgia,_serif]">
          ...
          {/* Modal Container */}
          <div className="relative w-full max-w-lg bg-neutral-900/80 backdrop-blur-2xl rounded-2xl border border-white/10 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="relative z-10 px-5 py-6">
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-gray-400 text-xs mt-0.5">We'll respond as soon as possible.</p>
                </div>
                <button onClick={() => setIsModalOpen(false)} className="p-1.5 hover:bg-white/10 rounded-full text-gray-400 hover:text-white transition-all">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form className="grid grid-cols-1 md:grid-cols-2 gap-3" onSubmit={handleMailRedirect}>
                <div className="space-y-1">
                  ...
                  <input 
                    type="text" 
                    required 
                    placeholder="SANTANU SAMANTA " 
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:ring-1 focus:ring-emerald-400/50 outline-none transition-all placeholder:text-gray-500" 
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <select 
                    className="w-full bg-neutral-800 border border-white/10 rounded-lg px-4 py-2 text-sm text-white outline-none cursor-pointer appearance-none"
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <textarea 
                    ...
                    className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:ring-1 focus:ring-emerald-400/50 outline-none transition-all resize-none min-h-[100px] h-28 placeholder:text-gray-500" 
                    onChange={(e) => setFormData({...formData, message: e.target.value})}
                  />
                </div>
                <button type="submit" className="md:col-span-2 w-full bg-emerald-500 hover:bg-emerald-400 active:scale-95 py-2.5 rounded-lg text-white font-bold text-sm transition-all shadow-lg shadow-emerald-500/20 mt-2">
                  Send Message
                </button>
              </form>
            </div>
